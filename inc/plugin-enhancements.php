 ...
 * @package genius
 */
class Genius_Theme_Plugin_Enhancements {
	/**
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Genius_Theme_Plugin_Enhancements;
		}
		return $instance;
	}
	/**
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features, including: ', 'genius' ),
					'<strong>' . esc_html__( 'Jetpack plugin', 'genius' ) . '</strong>' ),
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'site-logo' ) ) :
			$dependencies['logo'] = array(
				'name' => esc_html__( 'Site Logo', 'genius' ),
				'slug' => 'site-logo',
				'url'  => '',
				'module' => 'none',
			);
		...
		if ( current_theme_supports( 'featured-content' ) ) :
			$dependencies['featured-content'] = array(
				'name' => esc_html__( 'Featured Content', 'genius' ),
		...
		if ( current_theme_supports( 'jetpack-social-menu' ) ) :
			$dependencies['social-menu'] = array(
				'name' => esc_html__( 'Social Menu', 'genius' ),
		...
		if ( current_theme_supports( 'jetpack-testimonial' ) ) :
			$dependencies['testimonials'] = array(
				'name' => esc_html__( 'Testimonials', 'genius' ),
		...
		if ( current_theme_supports( 'jetpack-portfolio' ) ) :
			$dependencies['portfolios'] = array(
				'name' => esc_html__( 'Portfolios', 'genius' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => esc_html__( 'no specific module needed', 'genius' ),
			'custom-content-types' => esc_html__( 'Custom Content Types module', 'genius' ),
		);
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'genius_jetpack_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		$notice = '';
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s', 'genius' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'genius' ) . '</a>' : ''
				);
			}
			// Download message.
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please install %1$s. %2$s', 'genius' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'genius' ) . '</a>' : ''
				);
			}
			$notice .= '</p>';
		}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				esc_html__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'genius' ),
...
add_action( 'admin_head', array( 'Genius_Theme_Plugin_Enhancements', 'init' ) );

function enqueue_scripts() {
	// Add the admin JS if the notice has not been dismissed
	if ( is_admin() && get_user_meta( get_current_user_id(), 'genius_jetpack_admin_notice', true ) !== 'dismissed' ) {
		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'genius_jetpack_admin_script', get_template_directory_uri() . '/js/plugin-enhancements.js', array( 'jquery' ), '20160624', true );
		// Make some data available to our JS file
		wp_localize_script( 'genius_jetpack_admin_script', 'genius_jetpack_admin', array(
			'genius_jetpack_admin_nonce' => wp_create_nonce( 'genius_jetpack_admin_nonce' ),
		));
	}
}
...
function dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['genius_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['genius_jetpack_admin_nonce'], 'genius_jetpack_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'genius' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'genius_jetpack_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'genius' )
	) );
}
add_action( 'wp_ajax_genius_jetpack_admin_notice', 'dismiss_admin_notice' );